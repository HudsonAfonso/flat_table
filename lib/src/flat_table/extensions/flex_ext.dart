...
extension FlexColorExt on Color {
  ...
  Color brighten([final int amount = 10]) {
    ...
    final int value = (255 * -(amount / 100)).round();

    return Color.fromARGB(
      alpha,
      math.max(0, math.min(255, red - value)),
      math.max(0, math.min(255, green - value)),
      math.max(0, math.min(255, blue - value)),
    );
  }

  Color lightenOrDarken(BuildContext context, [final int amount = 10, bool inverse = false]) {
    final Brightness brightness = Theme.of(context).brightness;
    Color Function([int amount]) adjustFunc = brightness == Brightness.light ? darken : lighten;
    if (inverse) adjustFunc = brightness == Brightness.light ? lighten : darken;

    return adjustFunc(amount);
  }

  Color adjustBrightness(int amount, {bool isDarken = false}) {
    final Color Function([int amount]) adjustFunction = isDarken ? darken : lighten;

    return adjustFunction(amount);
  }
  ...
  Color blend(final Color input, [final int amount = 10]) {
    ...
    return Color.alphaBlend(input.withAlpha(amount * 255 ~/ 100), this);
  }
  ...
  Color getShadeColor({
    final int shadeValue = 10,
    final bool lightenn = true,
    ...
    if (this == Colors.black && !lightenn) return this;
    ...
    if (this == Colors.black) return lighten(usedShadeValue);

    // Trying to make white lighter, just return white
    if (this == Colors.white && lightenn) return this;
    ...
    return lightenn ? lighten(usedShadeValue) : darken(usedShadeValue);
  }
